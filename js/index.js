...
(function(){
    ...
    //用第三方函数继承
    function Fn(){}
    //注意：这里不是赋值，是引用关系               //注意！引用数据类型在比较时候，会判断它的路径是否相等，不同于
    Fn.prototype = Banr.prototype;            //于原始数据类型
    Banr2.prototype = new Fn();               //基础数据类型：string blood null undefined number
    Banr2.prototype.temp = Banr2.prototype.exe;//基础数据比较时候，只判断它是否长得一样，如果一样，就会返回turn
    Banr2.prototype.exe = function () {        //引用数据类型：object function arr
        this.temp();                           //引用数据在比较时候，会判断它的路径是否相等，如果相等，就会返回turn
        this.auto();
        this.clearTime();
    };
...
(function(){
    ...
    $goTop.click(function () {
        ...
        $("body,html").animate({
            ...
            /*
            * 是不是感觉懵逼！正常啦！我自己写的，过一个月回来看有时候都看懵逼！
            *
            *
            *
            *
